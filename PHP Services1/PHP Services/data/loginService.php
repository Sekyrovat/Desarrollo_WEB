<?php
	...
	else
	{
		...
		$uName = $_GET["username"];
				...
				** This is what line 42 does
				 * Alternative way of doing line 42
				 * if($_SERVER["REQUEST_METHOD"] == "GET")
				 * 	parse_str($_SERVER["QUERY_STRING"],$getVars)
				 * 	$uName = $getVars["username"]
				
				**
				 * The request method may be PUT, POST, GET, DELETE, you just change the method
				 * if($_SERVER["REQUEST_METHOD"] == "POST")
				 * {
				 		// We check the input of the ajax call
		...
		$sql = "SELECT fName,lName
				FROM users
				WHERE username = '$uName' AND passwrd = '$uPass'";
		...
		if ($result->num_rows > 0) {
			// This variable will be an array to keep the info of the result.
			// We loop through each row of the result
			while ($row = $result->fetch_assoc()) {
				// We are creating an array where we give the name to the value
													// The variable must be consistent with 
													// the column of the DB.
				$response = array('firstName' => $row["fName"], 'lastName' => $row["lName"]);
			}
			// We are going to send back a json, so we need to prepare it.
			echo json_encode($response);
			// And done. Just like that we have our json.
		} else {
