	...
	function attemptRegistration($username, $userFiName, $userLaName, $userEmail, $userPwd, $userGender, $userCountry)
	{
		...
		if ($conn) 
		{
			...
			else
			{
				...
				$prepared_stmt->bind_param("sssssss", $username, $userFiName, $userLaName, $userEmail, $password, $userGender, $userCountry);
		...
		if ($conn) 
		{
			...
			else 
			{
				...
				if ($prepared_stmt -> execute()) 
				{
					...
					$prepared_stmt ->bind_result($uId,$passInDb);
					$prepared_stmt -> fetch();
					$prepared_stmt -> close();
					$conn -> close();
					if (password_verify($pwd, $passInDb)) {
